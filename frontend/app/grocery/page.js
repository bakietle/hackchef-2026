...
export default function GroceryPage() {
  ...
  const { weekMode, shopItems, setShopItems } = usePlannerStore()
  const mode = WEEK_MODES.find(m => m.key===weekMode) || WEEK_MODES[0]

  const [items,  setItems]  = useState(() => {
    if (shopItems?.length > 0) return shopItems
  ...
  const toggle = (id) => {
    ...
    setItems(next); setShopItems?.(next)
  }
  ...
  const showToast = (msg) => { setToast({ msg, on:true }); setTimeout(() => setToast(t=>({...t,on:false})), 2200) }
  ...
  const enableNotifications = async () => {
    ...
    if (perm === 'granted') {
      ...
      setTimeout(() => new Notification('nomster 🍳', { body: `don't forget — ${items.filter(i=>!i.done).length} items to grab today!`, icon:'/icon.svg' }), 3000)
    } else {
    ...
    <div style={{ minHeight:'100vh', background:'var(--cream)', display:'flex', flexDirection:'column', paddingBottom:80 }}>
      ...
      <div style={{ position:'fixed', bottom:72, left:0, right:0, padding:'6px 20px', zIndex:40 }}>
        <button onClick={() => { showToast('week locked in! mama is proud 💕'); setTimeout(()=>router.push('/home'),900) }}
          style={{ width:'100%', padding:13, borderRadius:12, border:'2px solid var(--navy)', background:allDone?'var(--mint)':'var(--yellow)', color:'var(--navy)', fontSize:14, fontWeight:900, cursor:'pointer', fontFamily:'Nunito, sans-serif', boxShadow:'var(--shadow-md)' }}
