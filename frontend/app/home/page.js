    ...
    <div onClick={e => e.target===e.currentTarget && onClose()}
      ...
      <div style={{ background:'var(--cream)', borderRadius:'24px 24px 0 0', border:'2px solid var(--navy)', borderBottom:'none', padding:'22px 20px 40px', width:'100%', maxHeight:'88%', overflowY:'auto', animation:'slideUp .3s' }}>
        ...
        <div style={{ display:'flex', gap:12, alignItems:'flex-start', marginBottom:14 }}>
          <div style={{ width:64, height:64, background:`${modeColor}30`, border:'2px solid var(--navy)', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'center', fontSize:34, flexShrink:0 }}>{recipe.flag}</div>
          <div style={{ flex:1 }}>
        ...
        <div style={{ background:'var(--navy)', borderRadius:14, padding:'12px 14px', marginBottom:14 }}>
        ...
        <div style={{ background:`${modeColor}20`, border:'2px solid var(--navy)', borderRadius:14, padding:14, marginBottom:14 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:14 }}>
          {[['kcal',recipe.cal,'🔥'],['protein',recipe.pro,'🥩'],['carbs',recipe.carb,'🍞'],['fat',recipe.fat,'💧']].map(([k,v,ico]) => (
            <div key={k} style={{ background:'var(--white)', border:'2px solid var(--navy)', borderRadius:10, padding:'9px 6px', textAlign:'center' }}>
        ...
        {meals.length === 0 ? (
          ...
          meals.map(({ slot, recipe }) => (
            <div key={slot} onClick={() => onOpenRecipe(recipe)}
              ...
              <div style={{ width:48, height:48, background:'var(--yellow-l)', border:'2px solid var(--navy)', borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, flexShrink:0 }}>{recipe.flag}</div>
              <div style={{ flex:1 }}>
...
export default function HomePage() {
  ...
  const openDay = (dayName) => {
    ...
    const meals = SLOTS.map(slot => dayData[slot] ? { slot, recipe: dayData[slot] } : null).filter(Boolean)
    ...
    <div style={{ minHeight:'100vh', background:'var(--cream)', display:'flex', flexDirection:'column', paddingBottom:80 }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px 20px 0' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6 }}>
            ...
            <button onClick={() => router.push('/profile')}
              style={{ width:36, height:36, borderRadius:10, background:'var(--white)', border:'2px solid var(--navy)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
          ...
          <button onClick={() => router.push('/planner')} style={{ background:'transparent', border:'none', cursor:'pointer', fontSize:11, fontWeight:800, color:mode.color, fontFamily:'Nunito, sans-serif' }}>planner →</button>
        </div>
          ...
          {DAYS_SHORT.map((d, i) => {
            ...
            return (
              <div key={d} onClick={() => openDay(DAYS[i])}
                style={{ background:isToday?mode.color:'var(--white)', border:`2px solid ${dayMeals.length?'var(--navy)':'var(--border)'}`, borderRadius:12, padding:'6px 3px', textAlign:'center', cursor:'pointer', minHeight:68, boxShadow:isToday?'var(--shadow-sm)':'none', transition:'transform .15s' }}>
                ...
                {dayMeals.length > 0
                  ? dayMeals.slice(0,3).map((r,j) => <div key={j} style={{ fontSize:12, lineHeight:1.2 }}>{r.flag}</div>)
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            ...
            {pendingShop > 0 && <div style={{ background:'var(--coral)', color:'#fff', width:18, height:18, borderRadius:'50%', fontSize:10, fontWeight:900, display:'flex', alignItems:'center', justifyContent:'center', border:'1.5px solid var(--navy)' }}>{pendingShop}</div>}
          </div>
          <button onClick={() => router.push('/grocery')} style={{ background:'transparent', border:'none', cursor:'pointer', fontSize:11, fontWeight:800, color:mode.color, fontFamily:'Nunito, sans-serif' }}>see all →</button>
        </div>
        <div style={{ background:'var(--white)', border:'2px solid var(--navy)', borderRadius:14, padding:'10px 0', marginBottom:14, boxShadow:'var(--shadow-sm)', overflow:'hidden' }}>
          {shopList.slice(0,4).map((item, i) => (
            <div key={item.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 14px', borderBottom:i<3?'1px solid var(--border)':'none', cursor:'pointer', opacity:item.done?.6:1 }}>
        ...
        {/* Food tip */}
        <div style={{ background:'var(--yellow-l)', border:'2px solid var(--navy)', borderRadius:14, padding:'12px 14px', marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:4, marginTop:8 }}>
            {FOOD_TIPS.map((_,i) => <div key={i} onClick={() => setTipIdx(i)} style={{ width:i===tipIdx?20:6, height:4, borderRadius:2, background:i===tipIdx?'var(--navy)':'var(--border)', cursor:'pointer', transition:'all .3s' }}/>)}
          </div>
        </div>
        ...
        {/* COOK TOGETHER */}
        <button onClick={() => router.push('/mama?to=together')}
          style={{ width:'100%', padding:13, borderRadius:12, border:'2px solid var(--navy)', background:'var(--white)', color:'var(--navy)', fontSize:13, fontWeight:900, cursor:'pointer', fontFamily:'Nunito, sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:8, boxShadow:'var(--shadow-sm)', marginBottom:8 }}
          onMouseDown={e=>{e.currentTarget.style.transform='translate(2px,3px)';e.currentTarget.style.boxShadow='0 0 0 var(--navy)'}}
      ...
      {dayPopup && !recipePopup && (
        <DayPopup day={dayPopup.day} meals={dayPopup.meals} onClose={() => setDayPopup(null)} onOpenRecipe={r => setRecipePopup(r)}/>
      )}
      {recipePopup && (
