...
const FRIDGE_CATEGORIES = [
  { label:'Protein', emoji:'🥩', items:['Eggs','Chicken','Beef mince','Tuna','Tofu'] },
  { label:'Carbs',   emoji:'🍚', items:['Rice','Pasta','Bread','Potato','Noodles'] },
  ...
  { label:'Pantry',  emoji:'🫙', items:['Soy sauce','Olive oil','Canned beans','Lemon','Canned tomatoes'] },
]
...
export default function InputPage() {
  ...
  const { setMealPlan, weekMode, setShopItems } = usePlannerStore()
  ...
  const handleGenerate = () => {
    ...
    const iv = setInterval(() => {
      ...
      setProgress(pct)
      setWaitMsg(WAIT_MSGS[Math.min(si++, WAIT_MSGS.length-1)])
    }, 420)
    setTimeout(() => {
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', background:'var(--cream)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:40, textAlign:'center' }}>
      ...
      <h2 style={{ fontFamily:'Fraunces, serif', fontSize:22, fontWeight:900, marginBottom:8, color:'var(--navy)' }}>
        mama is cooking<br/>something up...
      </h2>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--cream)', paddingBottom:100 }}>
      ...
      {/* Header */}
      <div style={{ padding:'16px 20px 12px', display:'flex', alignItems:'center', gap:12, borderBottom:'1.5px solid var(--border)' }}>
      ...
      <div style={{ padding:'12px 20px', display:'flex', gap:10, alignItems:'center' }}>
        ...
        <div style={{ background:'var(--white)', border:'2px solid var(--navy)', borderRadius:'14px 14px 14px 4px', padding:'9px 13px', fontSize:12, color:'var(--navy)', lineHeight:1.6, fontWeight:600, boxShadow:'var(--shadow-sm)' }}>
          tell mama what u have and she'll cook up a plan for ur {mode.name} week!
        </div>
      </div>

      <div style={{ padding:'0 20px' }}>

        {/* ── FRIDGE SECTION — Tabbed categories ── */}
        <div style={{ marginBottom:18 }}>
          <div style={{ fontSize:10, fontWeight:800, color:'var(--muted)', textTransform:'uppercase', letterSpacing:1, marginBottom:10 }}>
            what's in ur fridge
            {selIng.length > 0 && (
              <span style={{ marginLeft:6, background:mode.color, color:'var(--navy)', padding:'2px 8px', borderRadius:8, fontSize:10 }}>
                {selIng.length} selected
              </span>
            )}
          </div>
          ...
          <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:4, marginBottom:10, scrollbarWidth:'none' }}>
            {FRIDGE_CATEGORIES.map(cat => (
              <button key={cat.label} onClick={() => setOpenCat(cat.label)} style={{
                flexShrink:0, padding:'6px 14px', borderRadius:20, fontSize:12, fontWeight:800,
                fontFamily:'Nunito, sans-serif', cursor:'pointer', border:'2px solid var(--navy)',
                background: openCat===cat.label ? 'var(--navy)' : 'var(--white)',
                color: openCat===cat.label ? mode.color : 'var(--muted)',
                boxShadow: openCat===cat.label ? 'var(--shadow-sm)' : 'none',
                transition:'all .12s',
              }}>
                {cat.emoji} {cat.label}
              </button>
          ...
          <div style={{ background:'var(--white)', border:'2px solid var(--navy)', borderRadius:16, padding:'12px 14px', boxShadow:'var(--shadow-sm)' }}>
            <div style={{ display:'flex', flexWrap:'wrap', gap:7 }}>
              {FRIDGE_CATEGORIES.find(c=>c.label===openCat)?.items.map(item =>
                chip(item, selIng.includes(item), () => toggle(item, setSelIng))
              )}
            </div>
          </div>
        ...
        <div style={{ display:'flex', gap:12, marginBottom:18 }}>
          {[
            ['budget per meal ($)', budget, setBudget, '8',  'number'],
            ['time available (min)', time,   setTime,   '20', 'number'],
        ...
        <div style={{ marginBottom:18 }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:7 }}>
            {CUISINES.map(c => chip(c, selC.includes(c), () => toggleC(c), 'var(--yellow-l)'))}
          </div>
        </div>
      ...
      <div style={{ position:'fixed', bottom:0, left:0, right:0, padding:'12px 20px 28px', background:'var(--cream)', borderTop:'2px solid var(--navy)' }}>
        <button onClick={handleGenerate} className="retro-btn"
          style={{ width:'100%', padding:15, background:'var(--yellow)', color:'var(--navy)', fontSize:15, fontWeight:900, boxShadow:'var(--shadow-md)' }}
