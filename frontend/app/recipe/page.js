...
import { regenerateRecipes } from '@/lib/api'
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: 18, fontWeight: 900, color: 'var(--navy)', marginBottom: 2 }}>
              {recipe.name}
            </div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              >
                {WEEK_MODES.find((m) => m.key === recipe.mode)?.emoji}{' '}
                {WEEK_MODES.find((m) => m.key === recipe.mode)?.name || recipe.mode}
              </span>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 900, color: 'var(--navy)' }}>
              {recipe.fuel}
            </div>
          </div>
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 6, fontWeight: 600 }}>
            {recipe.fuel >= 88
              ? 'excellent fuel — gonna smash it! 🚀'
              : recipe.fuel >= 80
              ? 'solid fuel — good energy ✨'
              : 'decent fuel — works for now 👌'}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 16 }}>
          {[
            ...
            >
              ...
              <div style={{ fontFamily: 'Fraunces, serif', fontSize: 14, fontWeight: 900, color: 'var(--navy)' }}>
                {v}
              </div>
              <div style={{ fontSize: 9, color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', marginTop: 1 }}>
                {k}
              </div>
            </div>
          ))}
        ...
        {recipe.steps.map((step, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--navy)', lineHeight: 1.65, paddingTop: 3, fontWeight: 500 }}>
              {step}
            </div>
          </div>
        ))}
        ...
        {recipe.miss?.length > 0 && (
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--navy)', marginBottom: 5 }}>
              🛒 still need to buy
            </div>
...
export default function RecipePage() {
  ...
  const handleRegenerate = async () => {
    if (!currentRequestId || loadingRegen) return

    try {
      setLoadingRegen(true)
      setError('')

      const regenerated = await regenerateRecipes(currentRequestId, {
        weekMode,
        budgetPerMeal: null,
        timePerMeal: null,
        cuisineType: 'Anything',
        dietary: '',
      })

      setMealPlan(regenerated.recipes)
    } catch (err) {
      setError(err.message || 'Could not regenerate recipes.')
    } finally {
  ...
  return (
    ...
    >
      ...
      {error && (
        <div style={{ padding: '0 20px 8px', color: 'var(--coral)', fontSize: 12, fontWeight: 700 }}>
          {error}
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 8px' }}>
        {all.length === 0 ? (
          ...
          >
            no recipes loaded yet. head back and generate a deck first.
          </div>
        ) : (
          all.map((r, i) => (
            <div
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'Nunito, sans-serif', fontSize: 14, fontWeight: 800, color: 'var(--navy)', marginBottom: 1 }}>
                  {r.name}
                </div>

                <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 5, fontWeight: 600 }}>
                  {r.cuisine} · {r.time}min · ${r.cost}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 9,
                    border: '2px solid var(--navy)',
                    background: savedRecipes.includes(r.id) ? 'var(--coral-l)' : 'var(--white)',
                    cursor: 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                ...
                >
                  <IconHeart
                    size={14}
                    color={savedRecipes.includes(r.id) ? 'var(--coral)' : 'var(--border)'}
                  />
                </button>
      ...
      >
        <button
          ...
          className="retro-btn"
          disabled={loadingRegen || !currentRequestId}
          style={{
            flex: 1,
            padding: 14,
            background: 'var(--white)',
            ...
            justifyContent: 'center',
            gap: 6,
            opacity: loadingRegen ? 0.7 : 1,
          }}
        >
          <IconRefresh size={14} color="var(--navy)" />
          {loadingRegen ? 'regenerating...' : 'not feeling it'}
        </button>
